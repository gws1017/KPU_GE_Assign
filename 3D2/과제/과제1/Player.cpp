	...
	nMeshes) : CPlayer(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, pContext, nMeshes)
{
	...
	pShader->CreateShaderVariables(pd3dDevice, pd3dCommandList);//여기가 문제였나?
	...
	SetPosition(XMFLOAT3(pTerrain->GetWidth() * 0.5f, 2000.0f, pTerrain->GetLength() * 0.5f)); //플레이어 초기 위치설정
