...
CMesh::~CMesh()
{
	...
	if (m_pd3dIndexBuffer) m_pd3dIndexUploadBuffer->Release();
		...
		pd3dCommandList)
{
	...
	//인덱스 버퍼 뷰를 생성한다. 
	IndexBufferView.BufferLocation = m_pd3dIndexBuffer->GetGPUVirtualAddress();
	...
	CMesh(pd3dDevice, pd3dCommandList)
{
	...
	nIndices = (nSlices * 3) * 2 + (nSlices * (nStacks - 2) * 3 * 2);
	m_pnIndices = new UINT[nIndices];
	k = 0;
	...
	float* pfNearHitDistance)
{
	...
	if (nIndices > 0) nPrimitives = (m_d3dPrimitiveTopology ==
