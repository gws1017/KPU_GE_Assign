...
void ReadObj(FILE *objFile )
{
	...
	int normalNum = 0;
	while (!feof(objFile)) {
		fscanf_s(objFile, "%s", count,sizeof(count));
	...
	normal = (glm::vec4 *)malloc(sizeof(glm::vec4)*normalNum);
		//--- 3. 할당된 메모리에 각 버텍스, 페이스 정보 입력
	while (!feof(objFile)) {
		fscanf_s(objFile, "%s", bind,sizeof(bind));
			...
			else if (bind[0] == 'vn' && bind[1] == '\0') {
				fscanf(objFile, "%f %f %f",
					&normal[faceIndex].x, &normal[faceIndex].y, &normal[faceIndex].z);
				normalIndex++;
			}
	}
}
