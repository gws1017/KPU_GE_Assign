...
GLboolean w = true, h = true, stop = true, open = false, rt = false, jm = false;
...
struct Stage {
	...
	void Draw(unsigned int modelLocation)
	{
		FF = FFRX * FFTZ;
		for (int i = 0; i < 2; i++)
		{
			glBindVertexArray(vao[i]);
			if(i== 0)glDrawElements(GL_TRIANGLES, CI, GL_UNSIGNED_INT, 0);
			if (i == 1)
			{
