...
glm::vec3 ltPos = glm::vec3(0.0f, 0.3f, -0.5f);
...
struct sphere {
	...
	void LoadOBJ()
	{
		loadOBJ("sphere.obj", vertices, uv, normals);
	}
	...
	void InitBuffer()
	{
		...
		S1 = glm::scale(S1, glm::vec3(0.003,0.003,0.003));
		S2 = glm::scale(S2, glm::vec3(0.005,0.005,0.005));
		S3 = glm::scale(S3, glm::vec3(0.009,0.009,0.009));
		S4 = glm::scale(S4, glm::vec3(0.01, 0.01, 0.01));
		T1 = glm::translate(T1, glm::vec3(-0.3, 0.0, 0.0));
		T2 = glm::translate(T2, glm::vec3(-0.6,0.0,0.0));
		T3 = glm::translate(T3, glm::vec3(-1.5, 0.0, 0.0));
	...
	void Rotate2()
	{
		R2 = glm::rotate(R2, glm::radians(speed * 1.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	}
	void draw(int i)
	{
		ltRy = glm::mat4(1.0f);
		...
		if (i == 0)
		{
			...
			glUniformMatrix4fv(modelLocation, 1, GL_FALSE, glm::value_ptr(R2 * T1* S1));
		}
		if (i == 1)
		{
			glUniform3f(vColorLocation, 0.0f, 0.8f, 0.0f);
			glUniformMatrix4fv(modelLocation, 1, GL_FALSE, glm::value_ptr(R2 * T2 * S2));
		}
		if (i == 2)
		{
			glUniform3f(vColorLocation, 0.0f, 0.0f, 0.8f);
			glUniformMatrix4fv(modelLocation, 1, GL_FALSE, glm::value_ptr(R2 * T3 * S3));
		}
		if (i == 3)
		{
...
struct Pyramid {
	...
	void LoadOBJ()
	{
		loadOBJ("pyramid.obj", vertices, uv, normals);
	}
	...
	void InitBuffer()
	{
		...
		S1 = glm::scale(S1, glm::vec3(0.1, 0.1, 0.1));
		T1 = glm::translate(T1, glm::vec3(0.0f, -0.5f, 0.0f));
	}
	void draw()
	{
		glBindVertexArray(vao);
		glUniform1i(flagLocation, 0);
		glUniform3f(vColorLocation, 0.8f, 0.8f, 0.0f);
		glUniformMatrix4fv(modelLocation, 1, GL_FALSE, glm::value_ptr(T1* S1));
...
struct Snow {
	...
	{
		random_device rd;
		uniform_int_distribution<int> uid(5, 10), uid2(-15,15);
		mt19937 mt(rd());
		speed = uid(mt);
		dy = 0.0f;
		pos[0] = 0.1 * uid2(mt);
	...
	void InitBuffer()
	{
		...
		S1 = glm::scale(S1, glm::vec3(0.0005, 0.0005, 0.0005));
		T1 = glm::translate(T1, glm::vec3(pos[0], 1.0f + dy * speed, pos[1]));
